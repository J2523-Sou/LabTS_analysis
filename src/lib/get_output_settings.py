...
def get_output_settings(landmarks):
    root = tk.Tk()
    root.title("出力設定")
    ...
    def submit():
        ...
        result = {
            ...
            "skeleton_video": video_output.get(),
            "landmarks": selected_landmarks,
        }

        if not any((result["csv"], result["coordinate_graph"], result["skeleton_video"])):
        ...
        if (result["csv"] or result["coordinate_graph"]) and not selected_landmarks:
            messagebox.showwarning(
                "出力設定", "出力するランドマークを1つ以上選択してください。", parent=root
            )
            result = None
            return
    ...
    tk.Checkbutton(root, text="骨格線入り動画", variable=video_output).pack(
        anchor="w", padx=30
    )

    tk.Label(root, text="出力するランドマーク（複数選択可）").pack(
        padx=20, pady=(15, 5)
    )
